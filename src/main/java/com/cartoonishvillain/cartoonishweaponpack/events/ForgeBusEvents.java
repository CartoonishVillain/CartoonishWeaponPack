        ...
        if(!event.player.level.isClientSide() && event.phase == TickEvent.Phase.END){
            event.player.getCapability(PlayerCapability.INSTANCE).ifPresent(h->{
                ...
                if (h.getCardEffectTicks() > 0) {
                    //play sound when effect ends.
                    if (h.getCardEffectTicks() == 1)
                        event.player.level.playSound(null, event.player.getOnPos(), SoundEvents.ZOMBIE_VILLAGER_CONVERTED, SoundSource.PLAYERS, 1, 1);
...
//            }
    ...
    public static void PlayerClickVanillaEvent(PlayerInteractEvent.RightClickItem event){
        if(!event.getEntity().level.isClientSide()){
            ...
            if((event.getItemStack().getItem().equals(Items.BRICK) || event.getItemStack().getItem().equals(Items.NETHER_BRICK)) && event.getHand() == InteractionHand.MAIN_HAND) {
                ThrowableItemProjectile throwingBrick = null;

                if (event.getItemStack().getItem().equals(Items.BRICK) && CartoonishWeaponPack.config.THROWBRICKS.get()) {
                    throwingBrick = new ThrowingBrick(Register.THROWINGBRICK.get(), event.getLevel(), event.getEntity());
                } else if (event.getItemStack().getItem().equals(Items.NETHER_BRICK) && CartoonishWeaponPack.config.THROWNETHERBRICKS.get()) {
                    throwingBrick = new ThrowingNetherBrick(Register.THROWINGNETHERBRICK.get(), event.getLevel(), event.getEntity());
                }
                if (throwingBrick != null) {
                ...
                if (event.getEntity().isCrouching() && CartoonishWeaponPack.config.THROWTNTCROUCH.get()) bombThrown = true;
                else if (event.getEntity().isFallFlying() && CartoonishWeaponPack.config.THROWTNTELYTRA.get()) bombThrown = true;
                ...
                if (bombThrown) {
                    ...
                    if (offhand.getItem().equals(Items.FIRE_CHARGE)) {
                        offhand.shrink(1);
                    } else {
            ...
            } else if(event.getItemStack().getItem().equals(Items.FIRE_CHARGE) && event.getHand() == InteractionHand.MAIN_HAND && CartoonishWeaponPack.config.FIRECHARGE.get()){
    ...
    public static void ItemToolTips(ItemTooltipEvent event){
        if ((event.getItemStack().getItem().equals(Items.BRICK) && CartoonishWeaponPack.config.THROWBRICKS.get())
                || (event.getItemStack().getItem().equals(Items.NETHER_BRICK) && CartoonishWeaponPack.config.THROWNETHERBRICKS.get())) {
            ...
            if (CartoonishWeaponPack.config.THROWTNTCROUCH.get()) {
                event.getToolTip().add(Component.translatable("cartoonishweapons.tnt.crouch").withStyle(ChatFormatting.BLUE));
            }
            if (CartoonishWeaponPack.config.THROWTNTELYTRA.get()) {
                event.getToolTip().add(Component.translatable("cartoonishweapons.tnt.crouch").withStyle(ChatFormatting.BLUE));
            }
        }
        else if (event.getItemStack().getItem().equals(Items.FIRE_CHARGE) && CartoonishWeaponPack.config.FIRECHARGE.get()){
